...
class PersistentVolume(ResourceBase):
    ...
    def get(self, fuzz_payload: list[str], fuzz_expression: str):
        ...
        print("persistent volume: get method fuzzing start.")
        ...
        print("persistent volume: get method fuzzing finish.")

    @exception_capture
    def post(self, fuzz_payload: list[str], fuzz_expression: str):
        # POST /api/v1/persistentvolumes
        print("persistent volume: post method fuzzing start.")
        options = ResourceBase.generate_fuzz_options("%s" % self.wfuzz,
                                                     f"-X POST " \
                                                     f'{" ".join(_ for _ in fuzz_payload)} ' \
        ...
        ResourceBase.api_caller(options)
        print("persistent volume: post method fuzzing finish.")

    @exception_capture
    def put(self, fuzz_payload: list[str], fuzz_expression: str):
        # PUT /api/v1/persistentvolumes/{name}
        print("persistent volume: put method fuzzing start.")
        options = ResourceBase.generate_fuzz_options("%s" % self.wfuzz,
                                                     f"-X PUT " \
                                                     f'{" ".join(_ for _ in fuzz_payload)} ' \
                                                     f'-H Content-Type:application/json ' \
        ...
        ResourceBase.api_caller(options)
        print("persistent volume: put method fuzzing finish.")
